...
import { Messages, NotificationTypes, Status } from '../common/constants';
...
export const createServiceTicket = (req: Request, res: Response, sio: any) => {
    ...
    parseFieldsAndUploadImageInS3(req, res, async (err: any, data) => {
        ...
        if (!params.source || !params.name || !params.email || !params.state || !params.phone) {
            return res.json({ status: Status.Error, message: `${Messages.MissingParams} (Source, Name, Email, State, Phone)` });
        }
