...
exports.getCustomers = (req, res) => {
    ...
    CompanyCustomer_1.CompanyCustomer.find({ company: companyId }, (err, companyCustomers) => {
        ...
        User_1.User.find({ _id: { $in: customerIds } }, 'info.email auth.email profile.firstName profile.lastName profile.displayName address.street address.city address.state address.zipCode contact.phone permissions.role', (err, users) => {
            if (err) {
                return res.json({ 'status': constants_1.Status.Error, 'message': constants_1.Messages.GenericError });
            }
